#!/bin/bash
#input are MASTER and SLAVE directories prepared by ISCE (till merged) and to be coregistered by doris
...
#if the area is too small, try with small coarsecorr window..
if [ $MLINES -lt 1000 ] || [ $MSAMPLES -lt 1000 ]; then
 sed -i '/CC_NWIN/d' coreg.dorisin
 sed -i '/CC_WINSIZE/d' coreg.dorisin
 sed -i '/STOP/d' coreg.dorisin
 echo "CC_NWIN     21" >> coreg.dorisin
 echo "CC_WINSIZE    64 64" >> coreg.dorisin
 echo "STOP" >> coreg.dorisin
fi
